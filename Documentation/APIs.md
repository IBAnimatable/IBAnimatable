...
| borderSides | Optional&lt;String> | border side: `top`, `right`, `bottom`, `left`, `topOut`, `rightOut`, `bottomOut`, or `leftOut` also can be found in emum [`BorderSides`](../IBAnimatable/BorderSides.swift). Multiple sides can be configured with a comma separated list of sides (e.g. `Top,Bottom`). If not specified, then display four sides. **Please notice**: If we use `maskType` property then `borderSides` will be ignored. |

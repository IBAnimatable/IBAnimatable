...
private extension FlipAnimator {
  ...
  func createSnapshot(fromView view: UIView, afterUpdates: Bool) -> [UIView] {
    ...
    guard let leftHandView = view.resizableSnapshotViewFromRect(snapshotRegion, afterScreenUpdates: afterUpdates, withCapInsets: UIEdgeInsetsZero) else {
      return []
    }
    ...
    guard let rightHandView = view.resizableSnapshotViewFromRect(snapshotRegion, afterScreenUpdates: afterUpdates, withCapInsets: UIEdgeInsetsZero) else {
      return []
    }

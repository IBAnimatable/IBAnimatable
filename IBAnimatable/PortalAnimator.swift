...
private extension PortalAnimator {
  ...
  func executeForwardAnimation(transitionContext: UIViewControllerContextTransitioning, containerView: UIView, fromView: UIView, toView: UIView) {
    guard let toViewSnapshot = toView.resizableSnapshotViewFromRect(toView.frame, afterScreenUpdates: true, withCapInsets: UIEdgeInsetsZero) else {
    ...
    guard let leftHandView = fromView.resizableSnapshotViewFromRect(leftSnapshotRegion, afterScreenUpdates: false, withCapInsets: UIEdgeInsetsZero) else {
      transitionContext.completeTransition(!transitionContext.transitionWasCancelled())
      return
    }
    ...
    guard let rightHandView = fromView.resizableSnapshotViewFromRect(rightSnapshotRegion, afterScreenUpdates: false, withCapInsets: UIEdgeInsetsZero) else {
      transitionContext.completeTransition(!transitionContext.transitionWasCancelled())
      return
    }
...
private extension PortalAnimator {
  ...
  func executeBackwardAnimation(transitionContext: UIViewControllerContextTransitioning, containerView: UIView, fromView: UIView, toView: UIView) {
    ...
    guard let leftHandView = toView.resizableSnapshotViewFromRect(leftSnapshotRegion, afterScreenUpdates: true, withCapInsets: UIEdgeInsetsZero) else {
      transitionContext.completeTransition(!transitionContext.transitionWasCancelled())
      return
    }
    ...
    guard let rightHandView = toView.resizableSnapshotViewFromRect(rightSnapshotRegion, afterScreenUpdates: true, withCapInsets: UIEdgeInsetsZero) else {
      transitionContext.completeTransition(!transitionContext.transitionWasCancelled())
      return
    }

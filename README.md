...
| borderSide | Optional&lt;String> | border side: `Top`, `Right`, `Bottom` or `Left`, also can be found in emum [`BorderSide`](https://github.com/JakeLin/IBAnimatable/blob/master/IBAnimatable/BorderSide.swift). Multiple sides can be specified with a comma separated list of sides (e.g. `Top,Right,Bottom`). If not specified, then display four sides. |
